...
2. **k-mer features**: counts canonical k-mers in each path sequence, sampled with closed syncmers by default (or all k-mers)
3. **node + edge dosage**: counts how many times each path traverses each node and each oriented step-to-step edge
...
It is the GWAS feature-extraction step: besides the per-bubble tables it writes, per substrate, a single pooled cohort genotype matrix in **BIMBAM mean-genotype dosage** format (`<out-dir>/bimbam_{kmers,graph}.bimbam.gz`) — the canonical genotype export, read directly by `panvar associate` and by tools like **GEMMA**. For a full, worked sample-level GWAS (copy number → quantitative trait, multiplicity vs presence/absence, Manhattan/QQ, traceback) see the **[GWAS example](../gwas_example.md)**. This dataset has been generated by Claude.
