...
Pass a [cosigt](https://github.com/davidebolo1993/cosigt) table (`sample <tab> hap1 <tab> hap2 …`) to `--samples <cosigt.tsv>` and `describe` also writes a per-sample BIMBAM for each substrate (under `sample/<substrate>/`) whose value sums the dosage over the sample's haplotypes (diploid `CN` = `CN_A` + `CN_B`). See [GWAS example](../gwas.md).

Outputs are transactional: everything `describe` owns is built in a sibling staging directory and moved into place only once every substrate pass has succeeded. A rerun with a different `--only-*`, `--bubble-id` or `--no-wide-matrix` setting removes the previous run's now-stale families rather than leaving them beside the new ones looking current; unrelated files in the output directory are left alone.
...
By default dosages are raw counts (not rescaled to 0–2), so a haplotype carrying 50 copies shows 50; `NA` = a haplotype that doesn't traverse the feature's bubble (distinct from `0` = traverses but reference). **A path taking the direct source→sink edge — a pure deletion — is a traverser**, and reads `0`, not `NA`.
