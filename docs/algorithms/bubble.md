...
For each snarl, the paths crossing `source → sink` (or `sink → source`, then canonicalized) are found, and the nodes each visits strictly between the boundaries are collected. The interior is the union of those nodes with the nodes the GRAPH carries between the boundaries — everything reachable from the near boundary's inner handle that also reaches the far boundary, over oriented handles. A branch no panel haplotype walks is still part of the site, and omitting it hid interior cycles from `--superbubbles`, which exists to exclude them. Two quantities are measured across those crossing paths: path support, the number of paths visiting both boundaries, and the interior span in bp, kept as its smallest and largest value. A path that does not visit both boundaries — a deletion that bypasses the locus, say — does not support the snarl.

### 4. Filter and merge

Base filters drop the uninteresting sites. `--min-variant-bp` keeps a snarl only if some supporting path has an interior span at least that large (the size filter, default 50); `--min-path-support` keeps only snarls crossed by at least that many paths. `--merge-nearby-bp` then optionally coalesces survivors that are consecutive IN REFERENCE COORDINATES, when the sequence strictly between the facing boundaries is within N bp — so two bubbles sharing a boundary have a gap of 0 regardless of how long that boundary node is. A fused bubble's interior is recomputed from the graph between its new outer boundaries rather than assembled from its parts, and the fusion is refused if any interior node with a reference coordinate falls outside the new span. The filters are then re-applied, because a fusion can exceed a bound its parts satisfied. Surviving sites are assigned bubble ids, in reference order.
