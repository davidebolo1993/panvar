...
- **Copy boundaries must fall exactly on node boundaries.** A copy that starts or ends inside a node cannot be folded without splitting that node, and rounding to the nearest boundary would delete the bases between the node edge and the copy edge — sequence *outside* the copy, which this mode is not licensed to discard. Such a copy is declined. What happens next depends on what the haplotype has left:

- **It still folds other copies** — the common case at a real array. Its copy number is undercounted by the declined copies and nothing else is wrong, so the site folds. Measured at LPA KIV-2: 8 copies are declined across 8 of 466 haplotypes, and all 466 still fold at least one copy (median 20), so the cost is about one copy in twenty for eight haplotypes.
- **It has no foldable copy at all** — then it would reach the site literally while its neighbours reach it through the REP node, and since copy number is counted from REP occurrences it would read **CN 0 while carrying copies**. That is a false zero rather than an undercount, so the whole site is refused (`status=partial_boundary`).

`--allow-partial-boundary` folds even in the second case, warning with the affected counts. Node splitting removes the trade-off entirely and is not implemented. That costs sensitivity where a builder did not cut at repeat boundaries, and it is the right trade for an option that emits a graph. Splitting nodes at aligned boundaries, keeping the prefix and suffix as fragments, is the fuller answer and is not implemented.
