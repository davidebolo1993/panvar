...
Some links cannot be dropped: another path still walks them, and a GFA link is global. That is the normal case at a real array — 2009 of them at LPA — so it is reported rather than refused. The question worth refusing on is the one those counts only hint at: **after the rewrite, can a rewritten path still be walked along its original route?** If every node and link of the replaced span survives for other reasons, the site is represented twice, folded and unfolded, and the re-snarl reports two alleles for one haplotype. That is checked directly on the delivered graph and refused, with `--allow-surviving-replaced-route` to accept it. At LPA 0 of 466 original walks remain walkable, because 2.9% of each one's steps name nodes that are gone.
