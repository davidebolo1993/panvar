...
- **Copy boundaries are rounded to node boundaries.** A copy that starts or ends inside a node causes the whole node to be replaced, so bases flanking the copy but sharing its node are lost. Measured: a 64 bp copy carried in a 70 bp node with 3 bp flanks loses both flanks. This is sequence outside the copy, and it is a defect rather than a documented lossiness; the fix is to split nodes at the aligned boundaries and keep the fragments.
