...
For the no-DUP and folded-DUP cases, each residual-segment position has its per-haplotype sequences collapsed to their exact distinct set and run through abPOA (one sequence, or none, is trivial). The region is skipped if a segment's longest sequence and, with --max-poa-work, the estimated DP-cell budget over the DISTINCT sequences abPOA actually receives. The resulting column MSA is compacted into a minimal sub-graph — a run of columns that all and only the same haplotypes traverse becomes one node, with fresh non-colliding ids — mapping each distinct residual sequence to a rebuilt node path.
