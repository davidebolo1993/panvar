...
Two length thresholds apply in sequence here, and the first is the one that bites. An alignment whose block length falls under `--min-align-len` contributes nothing at all: it is discarded as a whole before any of its differences are examined, so `--min-var` never sees them. The generator's native default for that threshold is set for whole-chromosome input, which a locus graph spanning a few tens of kb cannot reach — every alignment is then rejected, no event is inserted, and the result is the seed sequence as a single segment with no links, a graph in which every haplotype spells the same sequence. `auto` scales the threshold to half the seed haplotype, which leaves loci above the native default untouched and rescues the short ones. Because a generator can also legitimately return nothing, the outcome is checked rather than trusted: a graph with no links carries no variation, so it is discarded and the input is passed through unchanged.
