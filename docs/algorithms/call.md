...
`REP` applies where the array has been folded to a self-loop. `CN` is the exact number of traversals of that represented unit. `RU_LEN` is its per-copy length, so `(CN − REF_CN) × RU_LEN` gives the repeat-copy component of the size change. `CNBP` gives the full signed module-walk difference and `CNRESID = CNBP − (CN − REF_CN) × RU_LEN` exposes sequence in the same bubble that the copy delta does not explain. The traversal count is exact for the normalized graph; whether an approximate fold represents the intended biological unit must be validated separately.
