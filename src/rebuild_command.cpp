...
void print_rebuild_help() {
    std::cout
        ...
        << "consecutive steps are joined by links that exist, re-spelled and aligned against the original\n"
        << "above --min-recovered-identity. minigraph augments variation\n"
        ...
        << "      --min-matched-cover <X> ADVISORY, does not reject (default 0.95). Recovery is scored\n"
        << "                              from landmarks -- short seeds sampled along the haplotype --\n"
        << "                              and a seed occurring too often to locate anything is dropped.\n"
        << "                              A repeat-dense locus therefore leaves much of itself with no\n"
        << "                              usable landmark, and this fraction falls without any sequence\n"
        << "                              being lost. Below X the run says so and continues\n"
        << "  -r, --reference-path <name>  This path must be recovered within the identity bound. Exact match\n"
