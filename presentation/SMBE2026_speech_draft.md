...
## 0. Opening + the merge — slides [1]–[4] · 1:00

[1] Good morning, everyone an first off I would like to thank the organizers for having me here. It's my first time as a speaker at SMBE and I'm pretty much excited. My name's Davide, I serve as a bioinformatician at Human Technopole and I also act as Strategy and Operation consultant for a company also working in pangenome analyses.
This is the title of my talk as you see it in the agenda, but I've been asked by the organizers to rephrase it a bit in order to include also concurrent work Chiara from a PostDoc that works with me in the center - and i hope you haven't missed her poster (if so you still have time). Since my talk was mainly about genotyping from pangenomes and Chiara's about SV calling from graphs, to merge the two stories — so this is now a bit of a fusion, ya. Which is why the title changed: **genotyping, SV calling, and some preliminary association testing we are developing, all based on pangenomes.


## 1. COSIGT — the method — slides [5]–[9] · ~2:30

[5] Together with key collaborators, we developed this pipeline for genotyping using pangenome graphs called COSIGT. The aim of this pipeline is to assign a pair of structural haplotypes from a pangenome (what we call a genotype) to sequenced samples.

I'll quickly walk through it left to right. For a target locus we take the high-quality assembled haplotypes — say from HPRC/HGSVC — and build a **local pangenome graph**. This is done with PGGB and this is mainly work from Erik and Andrea.

Each haplotype in the graph is a path, and its node-coverage vector is just how many times it traverses each node (that's what you see here for some representative haplotypes in the pangenome). And this is relevant because often different coverage profiles reflects different haplotype structures - for instnace haplotypes with different copy numbers for a copy-number variable genes (you see an example for CYP2D6/D7 region here).

Then we take a short-read sample, align it to *all* haplotypes at once and get the read coverage over the same nodes. 

Now the trick: we can compare the coverage from every possible pair of haplotypes to the sample coverage vector using cosine-similarity and the best-matching haplotype pair is the genotype call. 

So genotyping becomes: which combination of known haplotypes best explains what I sequenced. The result is that we can then infer hapltoype structures that are routinely missed in a single linear reference especially in complex and structurally-variable regions.
