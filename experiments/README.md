...
## Bubble-local Cosigt falsifier

`cosigt_bubble_poc.py` applies Cosigt's weighted cosine to the same KIV-2 marker matrix, while
`cosigt_node_poc.py` applies it to exact graph-node traversal vectors and the real Panplexity mask.
Both select the same wrong content class, although marker counting reproduces the node-vector result
without read alignment. See `COSIGT_BUBBLE_POC.md` for the measurements and the recommended hybrid:
absolute mass/CN, normalized content shape and bounded junction evidence as separate local channels,
then the existing block HMM for mosaics.
